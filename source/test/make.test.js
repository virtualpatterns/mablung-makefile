...
;[
  ...
  Test(`cover ${variable}=... (dry-run)`, (test) => {
    ...
    test.true(stdout.includes('npx c8 --config configuration/cover.json ava --config test.js release/test/make.test.js'))
    test.true(stdout.includes('npx shx mv coverage ../Shared/mablung-makefile'))
...
Test('test (dry-run)', (test) => {
  ...
  test.is(result.code, 0)
  test.true(stdout.includes('npx ava --config test.js '))
...
;[
  ...
  Test(`test ${variable}=... (dry-run)`, (test) => {
    ...
    test.is(result.code, 0)
    test.true(stdout.includes('npx ava --config test.js release/test/make.test.js'))

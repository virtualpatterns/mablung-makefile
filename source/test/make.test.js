...
Test('default', (test) => {
  ...
  let message = null
  test.true(stdout.includes(message = `${Package.name}@${Package.version}`), `The output does not contain '${message}'.`)
...
Test('cover (dry-run)', (test) => {
  ...
  test.is(result.code, 0)
  test.true(stdout.includes('npx shx cp -R coverage ../Shared/mablung-makefile'))
